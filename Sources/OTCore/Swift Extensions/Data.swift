...
public let SystemEndianness: NumberEndianness = CFByteOrderGetCurrent() == CFByteOrderBigEndian.rawValue ?
...
extension Data {
	...
	public func toInt(from endianness: NumberEndianness = .platformDefault) -> Int? {
		return self.toNumber(from: endianness, toType: Int.self)
	}
...
extension Data {
	...
	public func toInt16(from endianness: NumberEndianness = .platformDefault) -> Int16? {
		return self.toNumber(from: endianness, toType: Int16.self)
	}
...
extension Data {
	...
	public func toInt32(from endianness: NumberEndianness = .platformDefault) -> Int32? {
		return self.toNumber(from: endianness, toType: Int32.self)
	}
...
extension Data {
	...
	public func toInt64(from endianness: NumberEndianness = .platformDefault) -> Int64? {
		return self.toNumber(from: endianness, toType: Int64.self)
	}
...
extension Data {
	...
	public func toUInt(from endianness: NumberEndianness = .platformDefault) -> UInt? {
		return self.toNumber(from: endianness, toType: UInt.self)
	}
...
extension Data {
	...
	public func toUInt16(from endianness: NumberEndianness = .platformDefault) -> UInt16? {
		return self.toNumber(from: endianness, toType: UInt16.self)
	}
...
extension Data {
	...
	public func toUInt32(from endianness: NumberEndianness = .platformDefault) -> UInt32? {
		return self.toNumber(from: endianness, toType: UInt32.self)
	}
...
extension Data {
	...
	public func toUInt64(from endianness: NumberEndianness = .platformDefault) -> UInt64? {
		return self.toNumber(from: endianness, toType: UInt64.self)
	}
...
extension Float32 {
	...
	public func toData(_ endianness: NumberEndianness = .platformDefault) -> Data {
		...
		withUnsafeBytes(of: &number) { rawBuffer in // UnsafeRawBufferPointer
			...
			case .littleEndian:
				switch SystemEndianness {
				case .littleEndian: data = Data(buffer: unsafeBufferPointer)
				case .bigEndian:    data = Data(Data(buffer: unsafeBufferPointer).reversed())
				default: fatalError() // should never happen
				}
				
			case .bigEndian:
				switch SystemEndianness {
				case .littleEndian: data = Data(Data(buffer: unsafeBufferPointer).reversed())
				case .bigEndian:    data = Data(buffer: unsafeBufferPointer)
				default: fatalError() // should never happen
				}
...
extension Data {
	...
	public func toFloat32(from endianness: NumberEndianness = .platformDefault) -> Float32? {
		...
		switch endianness {
		case .platformDefault:	return number
			
		case .littleEndian:
			switch SystemEndianness {
			case .littleEndian:	return number
			case .bigEndian:	return numberSwapped
			default: fatalError() // should never happen
			}
			
		case .bigEndian:
			switch SystemEndianness {
			case .littleEndian:	return numberSwapped
			case .bigEndian:	return number
			default: fatalError() // should never happen
			}
		}
	}
...
extension Data {
	...
	private func toNumber<T: FixedWidthInteger>(from endianness: NumberEndianness = .platformDefault, toType: T.Type) -> T? {
		if self.count != MemoryLayout<T>.size { return nil }
		...
		switch endianness {
		case .platformDefault:	return int
			
		case .littleEndian:
			switch SystemEndianness {
			case .littleEndian:	return int
			case .bigEndian:	return int.byteSwapped
			default: fatalError() // should never happen
			}
			
		case .bigEndian:
			switch SystemEndianness {
			case .littleEndian:	return int.byteSwapped
			case .bigEndian:	return int
			default: fatalError() // should never happen
			}
